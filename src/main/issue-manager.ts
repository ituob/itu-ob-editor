...
import {
  RunningAnnex,
  getRunningAnnexesForIssue,
  getAmendments,
} from "models/running-annexes";
...
> {
  ...
  rebuildRunningAnnexes(idx: Index<OBIssue>, forIssueID?: number) {
    if (forIssueID) {
      this.runningAnnexes[forIssueID] = getRunningAnnexesForIssue(
        forIssueID,
        idx
      );
    } else {
      for (const issueID of Object.keys(idx)) {
        this.runningAnnexes[issueID] = getRunningAnnexesForIssue(
          parseInt(issueID, 10),
          idx
        );
      }
  ...
  getLatestAnnex(
    ...
    asOfIssueID: number,
    excluding = false
  ): RunningAnnex | undefined {
    return (this.runningAnnexes[`${asOfIssueID}`] || []).filter(
      (annex) => annex.publicationID === ofPubID
    )[excluding ? 1 : 0];
  }

  // TODO: Super slow!
  async getDatasetChanges(
    forPubID: string,
    asOfIssueID: number
  ): Promise<DatasetChanges> {
    const latestAnnex = this.getLatestAnnex(forPubID, asOfIssueID);
    if (!latestAnnex) {
      log.warn(
        "Issue manager: Couldn’t fetch latest annex",
        forPubID,
        asOfIssueID
      );
      return {};
    ...
    const latestAnnexedDatasets =
      latestAnnex?.annexedTo.annexes[forPubID]?.datasets;
    if (latestAnnexedDatasets === undefined) {
      log.warn(
        "Issue manager: Couldn’t fetch dataset amendment changes",
        forPubID,
        asOfIssueID
      );
      return {};
    ...
    for (const dsID of Object.keys(latestAnnexedDatasets)) {
      ...
      for (const amd of amendmentsWithChanges) {
        changes[dsID].contents = [
          ...changes[dsID].contents,
          ...(amd.datasetChanges![dsID]?.contents || []),
        ];
      }
    }
  ...
  async autoFillDatasets(
    forPubID: string,
    asOfIssueID: number
  ): Promise<PositionDatasets | undefined> {
    const latestAnnex = this.getLatestAnnex(forPubID, asOfIssueID);
  ...
  public async readAll(_: { onlyIDs?: number[]; month?: Date }, force?: true) {
    if (!force) {
      log.error(
        "Cannot read all issues, too much data. Read schedule and individual issues instead."
      );
      return {};
    }
  ...
  async getCurrentIssueID() {
    ...
    const currentIssue: ScheduledIssue | null =
      issues
        .filter((item) => {
          return (
            new Date(item[1].publication_date).getTime() >= new Date().getTime()
          );
        })
  ...
  async savePublicationMeta(data: IssueMeta) {
    await this.db.update(PUBLICATION_META_PATH, data, [
      "issn",
      "languages",
      "authors",
    ]);
  ...
  async scheduleIssue(obj: ScheduledIssue) {
    ...
    try {
      publicationMeta = (await this.db.read(PUBLICATION_META_PATH, [
        "issn",
        "languages",
        "authors",
      ])) as IssueMeta;
  ...
  public setUpIPC(modelName: string) {
    ...
    listen<
      ...
      { annex: RunningAnnex | null }
    >(
      `${prefix}-get-latest-annex`,
      async ({ pubID, asOfIssueID, excluding }) => {
        return {
          annex: this.getLatestAnnex(pubID, asOfIssueID, excluding) || null,
        };
      }
    );
    ...
    listen<{ asOfIssueID: number }, { annexes: RunningAnnex[] }>(
      `${prefix}-get-running-annexes`,
      async ({ asOfIssueID }) => {
        return { annexes: this.getRunningAnnexes(asOfIssueID) };
      }
    );

    listen<{}, { id: number | null }>(
      `${prefix}-get-current-issue-id`,
      async () => {
