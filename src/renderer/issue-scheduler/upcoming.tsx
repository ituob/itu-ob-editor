...
export const UpcomingIssues: React.FC<UpcomingIssuesProps> = function ({
  issues,
  userIsEditing,
  currentIssueId,
}) {
            ...
            {existingIssues.map((issue) => (
              ...
              >
                <IssueScheduleCard
                  ...
                  onEditClick={() => app.openObjectEditor("issues", issue.id)}
                />
              </CSSTransition>
            ))}
          </TransitionGroup>
...
const DateStatus: React.FC<DateStatusProps> = function ({
  icon,
  className,
  dateClass,
  date,
  ...
  return (
    <div className={className}>
      <Icon iconSize={ICON_SIZE} icon={icon} className={dateClass} />
      <Text>{text}</Text> {date ? <DateStamp date={date} /> : "(no date)"}
    </div>
  );
};
...
const IssueScheduleCard: React.FC<IssueScheduleCardProps> = function ({
  issue,
  onEditClick,
  isCurrent,
  isNext,
  ...
  if (isCurrent) {
    bigIcon = (
      <Icon icon="play" htmlTitle="Publication in progress" intent="primary" />
    );
  } else if (isNext) {
    bigIcon = (
      <Icon icon="fast-forward" htmlTitle="Planned next" intent="primary" />
    );
  }
        ...
        <DateStatus
          ...
          text={
            <>
              ...
              <IssueWebLinks
                issueId={issue.id}
                pubDate={issue.publication_date}
              />
              ) on
            </>
          }
    ...
    >
      <header className={styles.issueInfo}>
        ...
        <div className={styles.scheduleInfo}>
          <div className={styles.publicationStatus}>{status}</div>
        </div>
      </header>

      <footer className={styles.actions}>
        {isCurrent ? (
          <Button fill={true} onClick={onEditClick} intent="primary">
            Edit
          </Button>
        ) : (
          <Button fill={true} onClick={onEditClick}>
            Open
          </Button>
        )}
      </footer>
...
const IssueWebLinks: React.FC<{ issueId: number; pubDate: Date }> = function ({
  issueId,
  pubDate,
}) {
  ...
  return (
    <span title="Edition is expected to be viewable online here. Newly added editions may take some time to appear online.">
      <a onClick={() => shell.openExternal(ituUrl)}>itu.int</a>,{" "}
      <a onClick={() => shell.openExternal(ituObUrl)}>ituob.org</a>
    </span>
  );
};
