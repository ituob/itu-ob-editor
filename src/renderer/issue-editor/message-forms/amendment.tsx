...
import {
  Message as AmendmentMessage,
  DatasetChanges,
} from "models/messages/amendment";
...
export const MessageForm: React.FC<MessageFormProps> = function ({
  ...
  >(
    "model-issues-get-dataset-changes",
    { changes: undefined },
    { forPubID: msg.target.publication, asOfIssueID: issue.id }
  );

  const latestAnnex = useIPCValue<
    { pubID: string; asOfIssueID: number; excluding: boolean },
    { annex: RunningAnnex | undefined | null }
  >(
    "model-issues-get-latest-annex",
    { annex: undefined },
    { pubID: msg.target.publication, asOfIssueID: issue.id, excluding: false }
  );

  const latestAnnexedDatasets =
    latestAnnex.value.annex?.annexedTo.annexes[msg.target.publication]
      ?.datasets || null;
    ...
    <FreeformContents
      ...
      onChange={(updatedDoc) => {
        if (onChange) {
          onChange({
            ...msg,
            contents: { ...msg.contents, [lang.default]: updatedDoc },
          });
        }
      }}
    />
  ...
  if (latestAnnexedDatasets === null) {
    ...
    return <NonIdealState icon={<Spinner />} title="Applying amendments…" />;
  } else {
    ...
    return (
      <div className={styles.normalizedAmendment}>
        ...
        <Tabs
          defaultSelectedTabId="changes"
          className={styles.changeMessageTabs}
        >
          <Tab
            id="changes"
            title="Review changes"
            panel={
              <AmendedPositionChangeReview
                datasets={datasetsWithAmendments}
                datasetChanges={changes}
              />
            }
...
export const AmendmentMeta: React.FC<{
  amendment: AmendmentMessage;
  issue: OBIssue;
    ...
    return (
      <>
        ...
        {latestAnnex && latestAnnex.positionOn ? (
          <div>
            Amending position of <DateStamp date={latestAnnex.positionOn} />{" "}
            annexed to OB {latestAnnex.annexedTo.id}:
          </div>
        ) : (
          <div>
            <Icon icon="warning-sign" /> This publication doesn’t seem to have
            been annexed to OB
          </div>
        )}
      </>
...
const AmendedPositionChangeReview: React.FC<AmendedPositionChangeReviewProps> =
  function ({ datasets, datasetChanges }) {
    const lang = useContext(LangConfigContext);
    ...
    const changes = Object.entries(datasetChanges)
      ...
      .map(([dsID, ds]) => {
        ...
        const dsTitle =
          (datasets[dsID]?.meta?.title || {})[lang.selected] || dsID;

        return nonTestOps.map((op) => (
          <Operation datasetTitle={dsTitle} op={op} />
        ));
      });
...
const Operation: React.FC<{ datasetTitle: string; op: Operation }> = function ({
  datasetTitle,
  op,
}) {
  return (
...
const AmendedPositionDataEditor: React.FC<AmendedPositionDataEditorProps> =
  function ({ datasets, datasetChanges, onChange }) {
    ...
    const [selectedDatasetID, selectDatasetID] = useState<
      (string & keyof typeof datasets) | null
    >(null);

    const selectedDataset =
      selectedDatasetID !== null ? datasets[selectedDatasetID] : null;
    const selectedPatched =
      selectedDatasetID !== null ? patched[selectedDatasetID] : selectedDataset;
            ...
            onChange
              ? (newContents) => {
                  if (selectedDatasetID !== null) {
                    const datasetContentChanges = jsonpatch.compare(
                      selectedDataset.contents,
                      newContents,
                      true
                    );
                    onChange(
        ...
        <div className={styles.navigation}>
          {Object.entries(datasets).map(([idx, dataset]) => (
            <ItemList
              ...
              title={
                dataset.meta.title
                  ? `“${dataset.meta.title[lang.selected]}”`
                  : idx
              }
              items={{
                contents: (
                  <>
                    Content&emsp;
                    <Tag round title="Dataset items">
                      {Object.entries(dataset.contents).length}
                    </Tag>
                  </>
                ),
              }}
