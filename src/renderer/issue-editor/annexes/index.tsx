...
function ({ pubId, position, onChange, issueId }) {
  ...
  if (!onChange && !position) {
    ...
    currentPosition = (
        <Button
          ...
          {position !== null ? <DateStamp date={position.position_on} /> : "Specify"}
        </Button>
    );
  }
  ...
  if (editingPosition) {
    ...
    dataEditor = <AnnexedPositionDataEditor
    ...
    const autoFillPossible = latestAnnex?.annexedTo.annexes[pubId]?.datasets !== undefined;

    dataEditor = <NonIdealState
    ...
    dataEditor = <NonIdealState
      ...
      description={onChange && !editingPosition
        ? <Callout intent="primary" style={{ textAlign: 'left' }}>
            <p>
              For an <abbr title="Service Publication">SP</abbr>
              {" "}
              that&nbsp;was or&nbsp;will&nbsp;be&nbsp;annexed multiple times to&nbsp;different OB&nbsp;issues,
              the&nbsp;specific&nbsp;position annexed&nbsp;to&nbsp;<em>this</em> OB&nbsp;issue must&nbsp;be specified.
            </p>
            <p>
              The&nbsp;annexed&nbsp;position is&nbsp;often (but&nbsp;not&nbsp;always) identical
        ...
        <div className={sharedStyles.editorMeta}>
          ...
          {position || (onChange && editingPosition)
            ? <FormGroup
                  ...
                  helperText={editingPosition ? "Annexed position is expressed as a date." : undefined}
                  label={<>
                ...
                {onChange && editingPosition
                  ? <DatePicker
                      ...
                      onChange={(newDate: Date | null) => {
                        if (newDate !== null) {
                          onChange(pubId, { ...(position || {}), position_on: newDate });
                        } else {
                          onChange(pubId, null);
                        }
